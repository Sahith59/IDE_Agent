...
# ── Config (sets HF_HOME + OLLAMA_MODELS from ~/.nexus/config.json) ──────────
...
# ── Paths (all from ~/.nexus/) ────────────────────────────────────────────────
HISTORY_DIR         = _cfg.HISTORY_DIR
USER_PLUGINS_DIR    = _cfg.USER_PLUGINS_DIR
BUILTIN_PLUGINS_DIR = _cfg.BUILTIN_PLUGINS_DIR
BM25_INDEX_PATH     = _cfg.BM25_INDEX_PATH
...
LOGO_FINAL_COLORS = [
    f"bold {PRI}", f"bold {ACC}", f"bold {PRI}",
    f"bold {ACC}", f"bold {PRI}", f"bold {YOU}",
]
...
class ChatSession:
    def __init__(self, session_id=None, model=None):
        ...
        self.model      = model or _cfg.get_default_model()
...
def render_model_selector():
    models  = _cfg.discover_models()
    default = _cfg.get_default_model()

    if not models:
        console.print(
            f"\n  [dim]Could not detect Ollama models automatically.[/dim]\n"
            f"  [dim]Make sure Ollama is running, or enter a model name:[/dim]\n"
        )
        name = console.input(f"[dim][[/dim] [{PRI}]Model[/{PRI}] [{PRI}]›[/{PRI}]  ").strip()
        return name or default

    rows = []
    for i, (model, size) in enumerate(models, start=1):
        marker   = f"  [bold {ACC}]← default[/bold {ACC}]" if model == default else ""
        size_str = f"  [dim]{size}[/dim]" if size else ""
        rows.append(
            f"  [bold {YOU}]{i}[/bold {YOU}]   "
            f"[bold {PRI}]{model:<28}[/bold {PRI}]{size_str}{marker}"
        )
    ...
    console.print(Panel(
        ...
        subtitle=f"[dim]  {len(models)} model(s) on your system  [/dim]",
        border_style=PRI, box=box.HEAVY, padding=(1, 2),
    ))
    console.print(f"  [dim]Enter 1–{len(models)+1}, or press Enter for {default}[/dim]")
    console.print()
    ...
    while True:
        ...
        err(f"Enter a number 1–{len(models)+1} or press Enter.")
...
def handle_pr_generator():
    try:
        from nexusai.pr_generator import run
        docs_parent = _cfg.NEXUS_HOME
        run(docs_parent)
...
def generate_tree_map(startpath):
    _skip = {".git", "node_modules", "venv", "__pycache__", "mac", "data", ".nexus"}
    tree  = []
...
def _execute_block(lang: str, code: str) -> None:
    ...
    confirm = console.input(f"  [dim][[/dim][{YOU}]y[/{YOU}][dim]/N] ›  [/dim]").strip().lower()
    if confirm != "y":
...
# ── /diagnose ─────────────────────────────────────────────────────────────────
def cmd_diagnose(error_msg: str, session, llm, workspace_dir: str) -> None:
    if not error_msg:
    ...
    for ref in dict.fromkeys(file_refs[:5]):
        fp = find_custom_file(os.path.basename(ref), workspace_dir) or \
    ...
    if len(injected) < 6000:
        for sym in dict.fromkeys(symbols[:6]):
            ...
            try:
                r = subprocess.run(["grep", "-r", "--include=*.py", "-l", sym, workspace_dir],
...
def load_plugins() -> None:
    ...
    for plugins_dir in (BUILTIN_PLUGINS_DIR, USER_PLUGINS_DIR):
...
# ── Agent mode ────────────────────────────────────────────────────────────────
def run_agent_loop(intent: str, session, llm, workspace_dir: str) -> str:
    ...
    scout_prompt = (
        ...
        f"Task: {intent}\n\n"
        f"Workspace:\n{generate_tree_map(workspace_dir)}\n\n"
    ...
    for rel in dict.fromkeys(file_matches[:6]):
        fp = os.path.join(workspace_dir, rel) if not os.path.isabs(rel) else rel
        if not os.path.exists(fp):
            fp = find_custom_file(os.path.basename(rel), workspace_dir) or ""
    ...
    if confirm == "d":
        for path, new_content in file_blocks:
            path = path.strip()
            full = os.path.join(workspace_dir, path) if not os.path.isabs(path) else path
            old_content = ""
...
[bold {PRI}]How to open Nexus in a specific project[/bold {PRI}]
  ...
  [dim]Just cd to your project and run nexus:[/dim]
  [bold {YOU}]cd /path/to/my/project[/bold {YOU}]
  [bold {YOU}]nexus[/bold {YOU}]

  Nexus will show an [bold]Active Project[/bold] banner confirming
...
[bold {PRI}]After every response[/bold {PRI}]
  ...
  [bold {YOU}]c[/bold {YOU}] → copies the full response to your clipboard.
  [bold {YOU}]r[/bold {YOU}] → if the response contained code, asks: run it? [y/N]
  [bold]Enter[/bold] → continue chatting.\
""",
        ),
        (
            "Working with Files",
...
[bold {PRI}]Files with spaces in the name[/bold {PRI}]
  ...
  PDFs are read automatically — text extracted from each page.

[bold {PRI}]View a file without injecting[/bold {PRI}]

  [bold {YOU}]/read /path/to/file[/bold {YOU}]   → syntax-highlighted panel
  [bold {YOU}]/ls /path/to/dir[/bold {YOU}]       → directory listing\
""",
        ),
        (
            "Project & Git Context",
...
[bold {PRI}]How Nexus understands your project[/bold {PRI}]
  ...
  Every session, Nexus automatically reads:

    [bold {YOU}]Directory tree[/bold {YOU}]   Up to 200 files and folders.
    [bold {YOU}]Git branch[/bold {YOU}]       The active branch name.
    [bold {YOU}]Recent commits[/bold {YOU}]   Last 8 commit messages.
    [bold {YOU}]Uncommitted diff[/bold {YOU}]  Everything changed but not committed.

  No setup needed. Nexus runs git CLI commands on your cwd.

[bold {PRI}]Pin files that are always relevant[/bold {PRI}]

  [bold {YOU}]/context add /path/to/important/file.py[/bold {YOU}]
  [bold {YOU}]/context show[/bold {YOU}]
  [bold {YOU}]/context rm 1[/bold {YOU}]
  [bold {YOU}]/context clear[/bold {YOU}]\
""",
        ),
        (
            "Sessions & History",
...
[bold {PRI}]Sessions are saved automatically[/bold {PRI}]
  ...
  Every conversation is saved in [dim]~/.nexus/history/[/dim].
  When you start Nexus, you'll see your past sessions.
  Pick a number to resume any of them.

[bold {PRI}]Organizing sessions[/bold {PRI}]

  [bold {YOU}]/rename my cool project[/bold {YOU}]
  [bold {YOU}]/tag python[/bold {YOU}]
  [bold {YOU}]/search authentication[/bold {YOU}]  → full-text search all sessions

[bold {PRI}]History and export[/bold {PRI}]

  [bold {YOU}]/history[/bold {YOU}]   → browse and read past sessions
  [bold {YOU}]/manage[/bold {YOU}]    → delete, rename, or clone
  [bold {YOU}]/export[/bold {YOU}]   → save session as Markdown to Desktop\
""",
        ),
        (
...
[bold {PRI}]Search the web from inside Nexus[/bold {PRI}]
  ...
  [dim]Inline in a message:[/dim]
  [bold {YOU}]@web:bitcoin price  — what's the latest price?[/bold {YOU}]

  Top 3 results are fetched and injected into the LLM context.

[bold {PRI}]Requires internet[/bold {PRI}]

  Web search uses DuckDuckGo. Everything else works fully offline.\
""",
        ),
        (
            "Power Tools",
...
[bold {PRI}]/diagnose — root-cause error analysis[/bold {PRI}]
  ...
  Paste an error or stack trace. Nexus scans your workspace for the
  files and symbols mentioned, reads them, and produces a diagnosis:
...
[bold {PRI}]Adding plugins[/bold {PRI}]
  ...
  Drop a [dim].py[/dim] file into [dim]~/.nexus/plugins/[/dim] — it becomes a slash
  command automatically on next launch. No config needed.

  [dim]~/.nexus/plugins/myplugin.py:[/dim]
  ...
  [dim]ctx[/dim] gives your plugin: [dim]session · llm · console · root_dir[/dim]

──────────────────────────────────────────────────

[bold {PRI}]Quick reference[/bold {PRI}]
...
def main():
    ...
    if not _cfg.is_setup_done():
        from nexusai.setup_wizard import run_wizard
        run_wizard()
    ...
    while not quit_entirely:
        ...
        # Workspace context (where the user launched nexus from)
            ...
            try:
                ...
                if user_input.startswith("/"):
                    ...
                    if cmd in ("exit", "quit"):
                        ...
                        if not arg:
                            ...
                            if not os.path.exists(rp):
                                ...
                                console.print(Panel(
                                    ...
                                    subtitle="[dim]  directory tree  [/dim]",
                                    border_style=PRI, box=box.HEAVY, padding=(1, 2)
                                ))
                            else:
                                try:
                        ...
                        cmd_diagnose(arg, session, llm, project_dir)
                        ...
                        try:
                            plugin["run"](arg, {
                                ...
                                "console": console, "root_dir": project_dir,
                            })
                        except Exception as e:
                ...
                if user_input.strip().startswith("@agent"):
                    ...
                    if not intent:
                        ...
                        continue
                    ai_response = run_agent_loop(intent, session, llm, project_dir)
                    if ai_response:
